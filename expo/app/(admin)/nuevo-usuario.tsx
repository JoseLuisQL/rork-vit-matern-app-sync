/**
 * Crear usuario (administración). Para una gestante, el servidor crea también
 * su ficha clínica y genera automáticamente el cronograma de 8 controles MINSA.
 * Requiere conexión (validación del servidor).
 */
import { useRouter } from "expo-router";
import { UserPlus, WifiOff } from "lucide-react-native";
import React, { useCallback, useState } from "react";
import { Alert, KeyboardAvoidingView, Platform, ScrollView, StyleSheet, Text, View } from "react-native";
import { adminTheme, common, radius, roleAccent, semantic, spacing, type } from "@/constants/theme";
...
const ROLES: Role[] = ["gestante", "obstetra", "admin"];

export default function NuevoUsuarioScreen(): React.ReactElement {
  const router = useRouter();
  const { adminCreateUser, online } = useApp();
      ...
      >
        ...
        >
          ...
          <View style={styles.roleRow}>
            {ROLES.map((r) => {
              const active = role === r;
              const colors = roleAccent(r);
              return (
                <PressableScale
                  key={r}
                  onPress={() => setRole(r)}
                  accessibilityLabel={`Rol ${ROLE_LABEL[r]}`}
                  style={[
                    styles.roleChip,
                    active
                ...
                >
                  <Text
                    style={[styles.roleText, { color: active ? common.white : common.textSecondary }]}
                  >
                    {ROLE_LABEL[r]}
                  </Text>
                </PressableScale>
          ...
          {role === "gestante" ? (
            <>
              ...
              <Field
                ...
                maxLength={10}
                accent={accent.primary}
                hint="Con esto el servidor calcula la edad gestacional, la FPP y el cronograma de 8 controles."
                testID="nuevo-fum"
              />
              <View style={styles.row2}>
                <Field
...
const styles = StyleSheet.create({
  ...
  offlineText: { ...type.bodySm, color: semantic.warning, flex: 1 },
