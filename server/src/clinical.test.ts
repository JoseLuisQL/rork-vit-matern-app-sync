...
describe("Server Clinical Engine — Constants & Setup", () => {
...
describe("Server Clinical Engine — Date & Validation Utilities", () => {
  ...
  it("validates day keys rigorously with isValidDayKey", () => {
    expect(isValidDayKey("2026-08-16")).toBe(true);
    expect(isValidDayKey("2024-02-29")).toBe(true); // Leap year
    expect(isValidDayKey("2026-02-29")).toBe(false); // Non leap year
    expect(isValidDayKey("2026-13-01")).toBe(false); // Invalid month
    expect(isValidDayKey("invalid-date")).toBe(false);
    expect(isValidDayKey("")).toBe(false);
  });

  it("adds days to a day key correctly, handling month rollovers", () => {
    expect(addDaysToKey("2026-01-30", 2)).toBe("2026-02-01");
...
describe("Server Clinical Engine — Gestational Age & FPP (Naegele Rule)", () => {
...
describe("Server Clinical Engine — Hemoglobin Altitude Correction & Anemia Classification", () => {
...
describe("Server Clinical Engine — Obstetric Risk Score Assessment (Semáforo)", () => {
